...
const AlreadySignedIn = ({ title }: { title: string }) => {
  ...
  return (
    <main className='sign-in-main'>
      <h1 className='sign-in-main-heading'>{title}</h1>
      <p className='sign-in-main-error'>You are already signed in!</p> {/* red error box */}
      <section>
        {/* style is for better text formatting */}
        <p
          className='sign-in-main-text'
          style={{ textAlign: 'justify', margin: '20px 48px 24px 48px' }}
        >
          An account is already signed in in this browser. Please choose on of the following options
          to proceed:
        </p>
        ...
        <button
          ...
          onMouseEnter={() => setOnHoverSignOut('brightness(0.82)')}
          onMouseLeave={() => setOnHoverSignOut('brightness(1)')}
        >
          {/* the onMouseEnter and -Leave is for the fade effect on hover which was not possible in css and the background animation. */}
          <div
            className='sign-in-main-button-background-gray'
            style={{
              transform: 'translate(-8px)',
              width: 'calc(100% + 16px)',
              filter: onHoverSignOut,
            }}
          />
        ...
        <button
          className='sign-in-main-button'
          style={{ backgroundColor: 'var(--color-transparent)' }}
          onMouseEnter={() => setOnHoverReturn('brightness(0.82)')}
          onMouseLeave={() => setOnHoverReturn('brightness(1)')}
        >
          {/* the onMouseEnter and -Leave is for the fade effect on hover which was not possible in css and the background animation. */}
          <Link to='/product'>
            <div
        ...
        <button
          ...
          style={{ backgroundColor: 'var(--color-transparent)' }}
          onClick={() => navigate('/')}
          onMouseEnter={() => setOnHoverContinue('brightness(0.75)')}
          onMouseLeave={() => setOnHoverContinue('brightness(1)')}
        >
          {/* the onMouseEnter and -Leave is for the fade effect on hover which was not possible in css and the background animation. */}
          <Link to='/'>
            <div
              className='sign-in-main-button-background'
              style={{
                transform: 'translate(-8px)',
                width: 'calc(100% + 16px)',
                filter: onHoverContinue,
              }}
            />
