...
const SignInPasswordReset = ({
  ...
  function displayError(errorMessage: string) {
    //function for displaying the error popup when password reset email fails
  ...
  function passwordReset(e: any) {
    ...
    sendPasswordResetEmail(firebase.auth, email).catch((error) => {
      //displays error if sign in fails
      switch (
  ...
  return (
    <section className='sign-in-password-reset' style={animationStyles}>
      ...
      {errorMessage && (
        <div className='sign-in-password-reset-error'>
          <span>{errorMessage}</span> {/*error message*/}
          <span className='sign-in-password-reset-error-close' onClick={() => setErrorMessage('')}>
            &#215;
          </span>{' '}
          {/*close popup button*/}
        </div>
      ...
      <p className='sign-in-password-reset-text'>
        If you lost or forgot your password and wish to reset it, you can use the form below.
      </p>
      ...
      <form onSubmit={passwordReset}>
        ...
        <input
          className='sign-in-password-reset-input'
          id='email'
          type='email'
        ...
        />
        <p className='sign-in-password-reset-text'>
          We will send a password reset link to the given email address.
        </p>
        ...
        <label>
          ...
          <div
            className='sign-in-password-reset-button'
